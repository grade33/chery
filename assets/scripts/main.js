...
;// CONCATENATED MODULE: ./src/scripts/vendor/range/SingleRange.js
function SingleRange_typeof(obj) { "@babel/helpers - typeof"; return SingleRange_typeof = "function" == typeof Symbol && "symbol" == typeof Symbol.iterator ? function (obj) { return typeof obj; } : function (obj) { return obj && "function" == typeof Symbol && obj.constructor === Symbol && obj !== Symbol.prototype ? "symbol" : typeof obj; }, SingleRange_typeof(obj); }
...
var SingleRange = /*#__PURE__*/function () {
  ...
  SingleRange_createClass(SingleRange, [{
    ...
    value: function updateProgress() {
      var max = this.rangeInput.getAttribute('max');
      var value = this.rangeInput.value;
      var percentage = value / max * 100;
      this.rangeProgress.style.width = "".concat(percentage, "%");
    }
  }, {
    key: "updateThumbPosition",
    value: function updateThumbPosition() {
      var maxValue = +this.rangeInput.getAttribute('max');
      var minValue = +this.rangeInput.getAttribute('min');
      var value = +this.rangeInput.value;
      var percentage = (value - minValue) / (maxValue - minValue) * 100;
      this.rangeThumb.style.left = "calc(".concat(percentage, "% - 15px)");
    }
  }, {
...
;// CONCATENATED MODULE: ./src/scripts/vendor/range/Range.js
  ...
  Range_createClass(Range, [{
    ...
    value: function init() {
      ...
      this.ranges.forEach(function (range) {
        _this.rangesInstances.push(new SingleRange(range));
      });
    }
  }]);
  return Range;
...
function initSwiperHero() {
  var classSwiper = classNames.swiper.hero;
...
function initSwiperGoods() {
  ...
  var classBtn = classNames.swiper.goods.btn;
  var classBtnActive = classNames.swiper.goods.btnActive;
  var buttons = document.querySelectorAll(".".concat(classBtn));
  var currentBtn = buttons[0];
  buttons.forEach(function (button) {
    button.addEventListener('click', function () {
      var slideIndex = button.dataset.slideIndex;
...
/* global ymaps */
function initYMap() {
  var address = 'Краснодар, ул. Ростовское шоссе, 34/10';
  var apiKey = '468db478-c4c3-4057-920e-2869b7bbb9d6';
  fetch("https://geocode-maps.yandex.ru/1.x/?apikey=".concat(apiKey, "&format=json&geocode=").concat(encodeURIComponent(address))).then(function (response) {
    return response.json();
  }).then(function (json) {
    var coords = json.response.GeoObjectCollection.featureMember[0].GeoObject.Point.pos.split(' ').map(parseFloat);
    var lat = +coords[1];
    var lon = +coords[0];
    var isMobile = window.innerWidth < 992;
    var mapCenter = isMobile ? [lat, lon] : [lat, lon + 0.005];
...
// Input Tel Mask
var classInputPhoneMask = classNames.inputPhoneMask;
var inputs = document.querySelectorAll(".".concat(classInputPhoneMask));
var maskOptions = {
